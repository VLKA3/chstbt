...
rhel7 ()	{
	...
	yum install squid httpd-tools -y
	# Asking user to set a username via read and writing it into $usrn
	echo "Your desired username: username" 
	# Creating user with username from $usrn and asking user to set a password
...
deb ()	{
	...
	# Asking user to set a username via read and writing it into $usrn
	echo "Your desired username: nhJ79nUh5OLW" 
	# Creating user with username from $usrn and asking user to set a password
	htpasswd -c /etc/squid3/passwd nhJ79nUh5OLW
	# Downloading Squid configuration
...
ubt ()	{
	...
	htpasswd -c /etc/squid/passwd $usrn
	# Downloading Squid configuration
	wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/hidden-refuge/squid-proxy-installer/master/spi-ubuntu.conf --no-check-certificate
